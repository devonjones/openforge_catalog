...
def test_blueprint_documentation_sql_operations(test_db):
    ...
    with test_db.pool.connection() as conn:
        with conn.cursor(row_factory=dict_row) as curs:
            ...
            curs.execute("""
                INSERT INTO blueprints (blueprint_name, blueprint_type, file_md5, file_name, config)
...
def test_tags_documentation_sql_operations(test_db):
    ...
    with test_db.pool.connection() as conn:
        with conn.cursor(row_factory=dict_row) as curs:
            ...
            # The tag is stored as array in DB but converted to pipe-delimited string by convert_tag_dict
...
def test_changelog_history_function(test_db):
    ...
    with test_db.pool.connection() as conn:
        with conn.cursor(row_factory=dict_row) as curs:
            ...
            curs.execute("""
                INSERT INTO blueprints (blueprint_name, blueprint_type, file_md5, file_name, successor_id, config)
