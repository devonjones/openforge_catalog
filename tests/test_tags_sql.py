...
def test_tag_search_blueprints(test_db):
    with test_db.connection() as conn:
        with conn.cursor(row_factory=dict_row) as curs:
            ...
            inserted_bp = blueprint_sql.insert_blueprint(curs, bp)
            tag_sql.insert_tag(curs, inserted_bp["id"], "foo|bar")
            results = tag_sql.tag_search_blueprints(curs, ["foo|bar"], [], [], None, None, 20, True, False, None)
...
def test_tag_search_tags(test_db):
    with test_db.connection() as conn:
        with conn.cursor(row_factory=dict_row) as curs:
            ...
            inserted_bp = blueprint_sql.insert_blueprint(curs, bp)
            tag_sql.insert_tag(curs, inserted_bp["id"], "foo|bar")
            results = tag_sql.tag_search_tags(curs, ["foo|bar"], [], [], None, None, 20, True, False, None)
            assert len(results) == 1
...
def test_tag_search_blueprint_images(test_db):
    with test_db.connection() as conn:
        with conn.cursor(row_factory=dict_row) as curs:
            ...
            inserted_bp = blueprint_sql.insert_blueprint(curs, bp)
            tag_sql.insert_tag(curs, inserted_bp["id"], "foo|bar")
            results = tag_sql.tag_search_blueprint_images(curs, ["foo|bar"], [], [], None, None, 20, True, False, None)
...
def test_tag_search_blueprint_count(test_db):
    with test_db.connection() as conn:
        with conn.cursor(row_factory=dict_row) as curs:
            ...
            inserted_bp = blueprint_sql.insert_blueprint(curs, bp)
            tag_sql.insert_tag(curs, inserted_bp["id"], "foo|bar")
            count = tag_sql.tag_search_blueprint_count(curs, ["foo|bar"], [], [], True, False, None)
            assert count == 1
...
def test_tag_search_tag_count(test_db):
    with test_db.connection() as conn:
        with conn.cursor(row_factory=dict_row) as curs:
            ...
            inserted_bp = blueprint_sql.insert_blueprint(curs, bp)
            tag_sql.insert_tag(curs, inserted_bp["id"], "foo|bar")
            count = tag_sql.tag_search_tag_count(curs, ["foo|bar"], [], [], True, False, None)
