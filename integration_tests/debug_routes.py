...
def debug_api_routes():
    ...
    try:
        response = requests.get(f"{base_url}/api/tags/texture/dungeon_stone/documentation", timeout=5)
        print(f"   GET /api/tags/texture/dungeon_stone/documentation: {response.status_code}")
        if response.status_code == 200:
            data = response.json()
            print(f"   Found {len(data.get('documentation', []))} documentation entries")
        elif response.status_code == 404:
            print("   ❌ Route not found - tag documentation routes may not be implemented")
    except Exception as e:
        print(f"   ❌ Error: {e}")
    ...
    try:
        ...
        response = requests.post(f"{base_url}/api/tags/test/debug/documentation", 
                               json=test_doc, timeout=5)
        print(f"   POST /api/tags/test/debug/documentation (no auth): {response.status_code}")
    except Exception as e:
        print(f"   ❌ Error: {e}")
    ...
    try:
        ...
        response = requests.post(f"{base_url}/api/tags/test/debug/documentation", 
                               json=test_doc, headers=headers, timeout=5)
        print(f"   POST /api/tags/test/debug/documentation (with auth): {response.status_code}")
        if response.status_code in [200, 201]:
            data = response.json()
            print(f"   Created documentation ID: {data.get('documentation', {}).get('id')}")
    except Exception as e:
        print(f"   ❌ Error: {e}")
    ...
    if blueprints:
        ...
        try:
            response = requests.get(f"{base_url}/api/blueprints/{blueprint_id}/documentation", timeout=5)
            print(f"   GET /api/blueprints/{blueprint_id}/documentation: {response.status_code}")
            if response.status_code == 200:
                data = response.json()
                print(f"   Found {len(data.get('documentation', []))} documentation entries")
        except Exception as e:
        ...
        try:
            ...
            response = requests.post(f"{base_url}/api/blueprints/{blueprint_id}/documentation", 
                                   json=test_doc, timeout=5)
            print(f"   POST /api/blueprints/{blueprint_id}/documentation (no auth): {response.status_code}")
        except Exception as e:
            print(f"   ❌ Error: {e}")
        ...
        try:
            ...
            response = requests.post(f"{base_url}/api/blueprints/{blueprint_id}/documentation", 
                                   json=test_doc, headers=headers, timeout=5)
            print(f"   POST /api/blueprints/{blueprint_id}/documentation (with auth): {response.status_code}")
            if response.status_code in [200, 201]:
                data = response.json()
                print(f"   Created documentation ID: {data.get('documentation', {}).get('id')}")
        except Exception as e:
    ...
    try:
        response = requests.get(f"{base_url}/api/blueprints/invalid-uuid/documentation", timeout=5)
        print(f"   GET /api/blueprints/invalid-uuid/documentation: {response.status_code}")
    except Exception as e:
        print(f"   ❌ Error: {e}")
    
    # Test invalid tag format
    try:
        response = requests.get(f"{base_url}/api/tags/invalid-tag-format/documentation", timeout=5)
        print(f"   GET /api/tags/invalid-tag-format/documentation: {response.status_code}")
    except Exception as e:
        print(f"   ❌ Error: {e}")
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"   Found {len(data.get('documentation', []))} total documentation entries")
    except Exception as e:
        print(f"   ❌ Error: {e}")
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"   Found {len(data.get('documentation', []))} documentation entries for texture")
    except Exception as e:
        print(f"   ❌ Error: {e}")
