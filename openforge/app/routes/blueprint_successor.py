    ...
    with current_app.db.connection() as conn:
        with conn.transaction():
            with conn.cursor(row_factory=dict_row) as cursor:
                ...
                try:
                    rowcount = blueprint_doc_sql.set_changelogs_non_live(cursor, successor_id)
                    if rowcount > 0:
                        logger.info(f"Set {rowcount} changelog(s) to is_live=false for successor {successor_id}")
