...
def create_session():
    ...
    try:
        ...
        session_token_hash = session_service._hash_token(session_data["session_token"])
        session = session_service._get_session_by_hash(session_token_hash)
        csrf_token = session_service._get_csrf_token_for_session(session['id'])
...
def delete_session():
    ...
    try:
        ...
        current_app.logger.error(f"Session deletion failed: {e}")
        return jsonify({"error": "Internal server error"}), 500 