...
def get_s3_client(config: Dict) -> boto3.client:
    ...
    endpoint_url = config.get("CLOUDFLARE_ENDPOINT") or os.environ.get("CLOUDFLARE_ENDPOINT")
    access_key_id = config.get("AWS_ACCESS_KEY_ID") or os.environ.get("AWS_ACCESS_KEY_ID")
    secret_access_key = config.get("AWS_SECRET_ACCESS_KEY") or os.environ.get("AWS_SECRET_ACCESS_KEY")
    
    # Validate required credentials
    if not endpoint_url:
        raise KeyError("CLOUDFLARE_ENDPOINT not found in config or environment variables")
    if not access_key_id:
        raise KeyError("AWS_ACCESS_KEY_ID not found in config or environment variables")
    if not secret_access_key:
        raise KeyError("AWS_SECRET_ACCESS_KEY not found in config or environment variables")
...
def get_s3_key_cache(s3_client: boto3.client, config: Dict, verbose: bool = False) -> Set[str]:
...
def upload_file(file_metadata: Dict, file_path: str, s3_client: boto3.client, 
                object_path: str, s3_key_cache: Optional[Set[str]] = None, config: Optional[Dict] = None, verbose: bool = False) -> str:
    ...
    Args:
        ...
        s3_key_cache: Optional cache of existing S3 keys (use get_s3_key_cache for best performance)
        config: Configuration dictionary containing S3_BUCKET_NAME
        verbose: Whether to print debug statements
    ...
    Returns:
        S3 object name
    """
    bucket = config.get("S3_BUCKET_NAME", "openforge-models") if config else "openforge-models"
    _, fn = os.path.split(file_path)
    parts = fn.split(".")
    extension = parts.pop()
    object_name = f"{object_path}/{file_metadata['md5'][:6]}/{file_metadata['md5']}.{extension}"
